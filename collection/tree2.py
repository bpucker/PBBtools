...
def main( arguments ):
	...
	if "--bootstrap" in arguments:
		bootstrapping_status = True
	else:
		bootstrapping_status = False
	...
	if tree_tool == "fast":
		...
		if bootstrapping_status:
			tree_file = prefix + ".raxml.support"
			os.popen( " ".join( [ raxml, "--all --threads 4 --model GTR+F+G --bs-trees 100 --msa", clean_cds_alignment_file, "--prefix", prefix ] ) )
			print " ".join( [ raxml, "--all --threads 4 --model GTR+F+G --bs-trees 100 --msa", clean_cds_alignment_file, "--prefix", prefix ] )
		else:
			tree_file = prefix + ".raxml.bestTree"
			os.popen( " ".join( [ raxml, "--all --threads 4 --model GTR+F+G --msa", clean_cds_alignment_file, "--prefix", prefix ] ) )
	...
	if '--taxon' in arguments:
		...
		renamed_cds_file = output_dir + name + ".cds.fasta"
		os.popen( " ".join( [ "python", renamefasta, "--in", mod_cds_FASTA, "--out", renamed_cds_file, "--taxon", taxon_file ] ) )
