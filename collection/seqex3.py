...
__usage__ = """
	...
	--flank <LENGTH_OF_FLANKING_SEQ>[500]
					"""
...
def collect_seq_parts( data, outputfile, content, flank_len, revcomp_state, name ):
	...
	with open( outputfile, "w" ) as out:
		for each in data:
			try:
				...
				if len( name ) > 0:
					out.write( '>' + name + "\t" + each['contig_name']  +  '_' + str( l_start ) + '_' + str( r_end ) + "\n" )
				else:
...
def main( arguments ):
	...
	if '--flank' in arguments:
		flank_len = int( arguments[ arguments.index('--flank')+1 ] )
	else:
		flank_len = 500
	
	collect_seq_parts( [ { 'contig_name': contig, 'start': start, 'end': end } ], outputfile, content, flank_len, revcomp_state, name )
